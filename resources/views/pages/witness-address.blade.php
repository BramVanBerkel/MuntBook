        ...
        <x-table-body>
            @foreach($transactions as $transaction)
                <x-table-row color="{{ ($loop->even) ? 'bg-gray-50' : 'bg-white' }}">
                    <x-table-data-item>
                        <x-link rel="nofollow"
                                href="{{ route('transaction', ['txid' => $transaction->transaction->txid]) }}">
                            <x-date :date="$transaction->transaction->created_at"/>
                        </x-link>
                    </x-table-data-item>
                    <x-table-data-item>
                        <x-gulden-display
                            value="{{ $transaction->transaction->vouts()->firstWhere('n', '=', 1)->value }}"/>
                    </x-table-data-item>
                </x-table-row>
            @endforeach
        </x-table-body>
